...
Figuring all this out is the job of the protocol stack (e.g., TCP) in
...
with bit 0 and working down to bit 31, usually eight bits at a time.

A lot of the computers on the Internet use little endian encoding,
...
order along the path from RAM to NIC.
...
each with one ounce in it.

The way envelopes go through the mailing system, you're better off not
 ...
 * Identification: This is a serial number, generated by the sending
   ...
   stack correctly reassemble the message.
 ...
 * Fragmentation Offset: Used with the Identification sequence number,
   ...
   follow this one when re-assembling the message.
 * Time to Live (TTL): This indicates the number of routers that a
   ...
   like 255 without any real bottlenecks.
 ...
 * Header Checksum: This calculates a checksum for the header
   only. It's only used in IPv4. Doing integrity-checking on the data
   is the responsibility of Layer 4.
 * Source Address: This is the IP address of the sender of the packet,
   ...
   change this address so they can get the answer back.
